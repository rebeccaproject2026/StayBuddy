...
export default function MobileActionBar({ property, language, isAuthenticated, isOwner, t, onContact, onShare, onLoginRedirect }: Props) {
  const handleCall = () => {
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 shadow-2xl px-4 py-3 flex gap-3">
      ...
      <button onClick={onShare}
        className={`${isOwner ? "flex-1" : "w-12"} py-3 border-2 border-gray-300 hover:border-primary text-gray-700 rounded-xl transition-colors flex items-center justify-center gap-2`}>
        <Share2 className="w-5 h-5" />
        {isOwner && <span className="text-sm font-semibold">{t.share}</span>}
      </button>
    </div>
  );
}
