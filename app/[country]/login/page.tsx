      ...
      >
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl p-5 sm:p-7 md:p-8">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-5">
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6, duration: 0.4 }}
                ...
                <label className="flex items-center cursor-pointer gap-2">
                  <input {...register("rememberMe")} type="checkbox" disabled={isLoading}
                    className="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary cursor-pointer disabled:opacity-50" />
                  <span className="text-xs sm:text-sm text-gray-600">{t("login.rememberMe")}</span>
                </label>
                <Link href="/forgot-password" className="text-xs sm:text-sm text-primary hover:text-primary-dark font-medium transition-colors whitespace-nowrap">
              ...
              <motion.button type="submit" disabled={isLoading}
                ...
                {isLoading ? (<><Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" />{language === 'fr' ? 'Connexion...' : 'Signing in...'}</>) : t("login.button")}
              </motion.button>
            </form>

            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8, duration: 0.4 }} className="mt-5">
              <div className="relative">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.4 }} className="mt-4">
              <motion.button onClick={handleGoogleLogin} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                className="w-full py-2.5 sm:py-3 bg-white border border-gray-300 text-gray-700 rounded-xl font-semibold shadow hover:bg-gray-50 transition-all duration-300 flex items-center justify-center gap-3 text-sm sm:text-base">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.0, duration: 0.4 }} className="mt-4 text-center">
              <p className="text-sm sm:text-base text-gray-600">
                {t("login.noAccount")}{" "}
                <Link href="/signup" className="text-primary font-semibold hover:text-primary-dark transition-colors">{t("login.signupLink")}</Link>
              </p>
            </motion.div>
          </div>
        </motion.div>
        ...
        {showRoleSelection && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-center text-primary mb-3">Choose Your Role</h3>
              <p className="text-sm text-gray-600 text-center mb-6">Please select whether you're looking to rent or list properties.</p>
              <div className="space-y-3">
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleRoleSelect('renter')}
                  className="w-full py-3 bg-primary text-white rounded-xl font-semibold hover:bg-primary-dark transition-all flex items-center justify-center gap-3 text-sm sm:text-base">
                  <span>🏠</span> I'm a Renter
                </motion.button>
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleRoleSelect('landlord')}
                  className="w-full py-3 bg-accent text-white rounded-xl font-semibold hover:bg-accent-hover transition-all flex items-center justify-center gap-3 text-sm sm:text-base">
                  <span>🏢</span> I'm a Landlord
                </motion.button>
              </div>
              <button onClick={() => setShowRoleSelection(false)} className="w-full mt-3 py-2 text-sm text-gray-500 hover:text-gray-700 transition-colors">Cancel</button>
            </motion.div>
          </motion.div>
        )}
      </div>
